...
- (NSString*)encodePercentEscapesStrictlyPerRFC2396 {
  ...
  // The above may return NULL if url contains invalid escape sequences like
  // %E8me, %E8fe, %E800 or %E811,
  // because CFURLCreateStringByReplacingPercentEscapes() isn't smart enough to
  // ignore them.
  ...
  // So, we recover from this rare but possible error by returning the original
  // self because it's "better than nothing".
...
- (NSString*)decodeAllPercentEscapes {
  // Unfortunately, CFURLCreateStringByReplacingPercentEscapes() seems to only
  // replace %[NUMBER] escapes
  NSString* cfWay =
    (NSString*)[(NSString*)
