...
{	
	...
	if (v) {
		...
		[str appendFormat:@"+%@ <%s %x> retain:%d - tag:%d - bgcolor:(%@) - opaque:%@\n"
		 @"%@ bounds: x:%.0f y:%.0f w:%.0f h:%.0f - frame: x:%.0f y:%.0f w:%.0f h:%.0f - center: x:%.0f, y:%.0f\n", 
     indentString1, class_getName([v class]), v, v.retainCount, v.tag, [self listColor:v.backgroundColor], self.opaque ? @"YES" : @"NO",
